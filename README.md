...
- DDL : SQL
    - CREATE - TABLE, VIEW, PROCEDURE, FUCTION, 개체를 생성하는 키워드
        - 타입형
        CREATE TABLE 테이블명 (
            첫번째_컬럼   타입형  제약조건,
            두번째_컬럼   타입형  제약조건,
...
- DML : [INSERT쿼리](./day04/sql01_INSERT.sql), [UPDATE/DELETE쿼리](./day04/sql02_UPDATE_DELETE.sql)
    - INSERT - 체이블에 새로운 데이터를 삽입하는 명령
        ...
        ```
        - 여러건 한꺼번에 삽입
    - UPDATE - 데이터 변경. WHERE조건 없이 실행하면 테이블 모든 데이터가 수정됨(주의요망!)
        ```sql
        UPDATE 테이블명 SET
            컬럼명 = 변경할 값
            [컬럼명 = 변경할 값] -- 반복
        [WHERE 조건]
        ```
    - DELETE - 데이터 삭제. WHERE조건 없이 실행하면 테이블 모든 데이터가 삭제됨(주의요망!)
        ```sql
        DELETE FROM 테이블명
        [WHERE 조건];
        ```
- 트랜잭션 : [트랜잭션쿼리](./day04/sql03_트랜잭션.sql)
    ...
    - 은행에서 돈을 찾을 때 아주 많은 테이블에 접근해서 일을 처리
        ...
        - 제대로 일이 처리 안 되면 다시 원상복귀
        - atm 기계정보테이블, 회원테이블에서 비밀번호 확인, 계좌정보, 계좌금액입출금내역테이블, 
        - 계좌정보테이블, 계좌금액입출금내역테이블금액빼기, ATM기계총지폐정보테이블,
        영수증관련정보테이블, atm기계정보테이블, 은행로그테이블
        - db 설정 AUTO COMMIT 해제하는걸 권함
        - ROLLBACK 트랜잭션 종료가 아님. COMMIT만 종료.
        ```sql
        BEGIN TRANSACTION; -- 트랜잭션 시작(옵션)
        COMMIT; --트랜잭션 확정
        ROLLBACK; -- 원상복귀
        ```
- 제약조건(Constraint) : [제약조건쿼리](./day04/sql04_제약조건.sql)
    - 잘못된 데이터가 들어가지 않도록 막는 기법
        - PK(Primary Key) - 기본키, UNIQE NOT NULL. 중복되지 않고 없어도 안됨
        - FK(Foriegn Key) - 외래키, 다른 테이블 PK에 없는 값을 가져다 쓸 수 없음
        - NOT NULL - 값이 빠지면 안 됨
        - UNIQUE - 들어간 데이터가 중복되면 안 됨
        - CHECK - 기준에 부합하지 않는 데이터는 입력되면 안 됨
        - DEFAULT - NULL 입력시 기본값이 입력되도록 하는 제약조건건
        ```SQL
        CREATE TABLE 테이블명 (
            컬럼 생성시 제약조건 추가
        );

        ALTER TABLE 테이블생성 ADD CONSTRAINT 제약조건
        ```
- INDEX : [INDEX쿼리](./day04/sql05_인덱스.sql), [인덱스용테이블생성](./ref/bulk_data_insert.sql)
    - 책의 찾아보기와 동일한 기능
    - 검색을 매우 빨리 할 수 있도록 해줌
    - B(alanced) Tree를 사용해서 검색횟수를 log(n)건으으로 줄임
    - 인덱스 종류
        - 클러스터드(Clustered) 인덱스 (테이블 당 1개)
            - PK에 자동으로 생성되는 인덱스(무지 빠름)
            - PK가 없으면 처음으로 설정되는 UNIQUE 제약조건의 컬럼에 생성
        - 보조(None-Clustered) 인덱스 (여러개)
            - 사용자가 추가하는 인덱스
            - 클러스터드 인덱스보다 조금 느림
        - 유의점
            - PK에 자동 인덱스 후 컬럼에 UNIQUE를 걸어도 인덱스가 생성 안 됨. 수동으로 생성 필요
            - WHERE절에서 검색하는 컬럼은 인덱스를 걸어주는 것이 성능향상에 도움이 됨
            - 인덱스는 한 테이블당 4개이상 걸면 성능 저하
            - NULL값이 많은 컬럼에 인덱스는 성능 저하 (NULL도 중복된 데이터이기 때문)
            - INSERT, UPDATE, DELTE가 많이 발생하는 테이블에 인덱스를 걸면 성능 저하
            ```sql
            CREATE INDEX 인덱스명 ON 테이블명(인덱스를 걸 컬럼명)
            ```

## 5일차  
- VIEW : [VIEW](./day05/sql01_뷰.sql)
    - 기존 테이블에서 권한별로 보일 수 있는 컬럼을 지정해서 만드는 개체(보안목적)
    - 기존 테이블 중 개인정보나 중요한 부분이 있으면 제외하고 보일 수 있음
    - view라도 INSERT, UPDATE, DELETE가 가능함. 단일뷰에서만
        ```SQL
        CREATE VEIW 뷰명
        AS
            SELECT 쿼리;
...
- 시퀀스 : [시퀀스](./day05/sql03_시퀀스.sql)
    ...
    - Oracle만 존재. 타 DB보다 자동증가값 사용 불편편
        ...
        [CACHE] -- 번호증가 캐시(대용량 삽입시만 관계)
        ```
- 사용자 계정 전환 : [쿼리](./day05/sql04_사용자계정관리.sql)
...
- Stored Procedure와 Function을 만들기 위해서 사용
    - 저장프로시저
        - 한꺼번에 많은 일을 수행해야할 때(Transaction당 수행되는 로직을 묶어서)
        ...
        - 실행시 EXEC 사용
        ```sql
        exec 르시저명(파라미터); -- DBeaver에서 사용불가
        ```

    - 함수
        - 스칼라값을 리턴할 때 -Seledt절 서브쿼리와 기능이 동일
