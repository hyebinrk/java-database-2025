...
-- 03. 02번의 쿼리를 컬럼명을 변경해서 출력하세요. '직원', '이름', '성', '이메일', '입사일자'로 변경하시요(107행)
...
-- 04. employees에서 job_id가 중복되지 않게 출력하시요.(19행)
-- distinct - 중복제거, all - 전부 다
SELECT DISTINCT job_id
	FROM employees;

--05.emplyees 테이블의 이름과 성을 붙여서 full_name으로, phone_number를 출력하시요.(107행)
 ...
 * 조건절 - WHERE
 * */
-- 연습. emplyees 테이블에서 employee_id가 110인 직원을 출력하세요.(1행)
SELECT *
 FROM employees
 ...
 * ● !=, <> : not equal. 같지 않다
 * ● >, >=, <, <= : 크다, 크거나 같다, 작다, 작거나 같다
 * ● BETWWEN a AND b : 특정 값이 a와 b 사이에 있다.
 * ● IN(a, b, c) : 특정 값이 a이거나 b이거나 c 중 하나임
...
-- % 앞에 무슨 글자들이 있는지 상관없이
 SELECT *
  FROM employees
  WHERE job_id LIKE '%MAN'; -- 앞쪽 문자열은 뭐든지 상관없고 MAN으로 끝나는 문자열을 가진 패턴 찾아라
 ...
 SELECT *
  FROM employees
  WHERE last_name LIKE 'Ra__'; -- 총 네글자 중 Ra로 시작하는 패턴을 찾아라
  
 SELECT *
  FROM employees
  WHERE job_id LIKE '%MAN%'; -- 문자열 내 어디든지 MAN이 들어가는 문자열은 다 찾아라
  
-- 07. exployees에서 hire_date가 2005-0101 이후에 입사했고, salary가 10000이상인 직원을 출력하시오.
--		이름과 이메일, 전화번호, 입사일, 급여를 표시하시오.
 SELECT first_name
		, last_name
		, phone_number
		, hire_date
...
-- employees에서 employee_id가 110에서 150 사이인 직원아이디와 직원명(first_name || last_name)을 합쳐서 출력하시오.
