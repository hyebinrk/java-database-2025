...
-- where절 서브쿼리는 다중행이 되면 안됨(같지않다<>도 안됨)
SELECT s.name, d.dname 
	FROM STUDENT s, department d
	WHERE s.deptno1 = d.deptno
	  AND s.deptno1 = (SELECT deptno1 
...
-- ECISTS 서브쿼리의 값이 있는 겨웅 메인쿼리를 수행
...
-- 부서의 지역의 Pohang Main Office인 부서코드에 해당하는 직원들의 사번, 이름, 부서번호를 출력하시오.
...
SELECT e.name, e.empno, e.POSITION, e.deptno, e.pay, g1.payavg
	  ...
	  FROM emp2 e, g1 -- g는 가상테이블
	   WHERE e.deptno =  g1.deptno;

-- 11번 12번은 차이가 없음
-- where절 서브쿼리 > from절 서브쿼리 > select절 서브쿼리(사용자정의함수로 대체할 수 있기 때문에 많이 안 씀) 의 순으로 많이 쓴다.
