# Oracle Student앱
# DRUD 데이터베이스 DML(SELECT, INSERT, UPDATE, DELETE)
## CREATE(INSERT), REQEUST(SELECT), UPDATE(UPDATE), DELETE(DELETE)
import sys
...
class MainWindow(QMainWindow):
    ...
    def btnAddClick(self):
        ...
        if std_name == "" or std_regyear == "":
            ...
            QMessageBox.warning(self,'경고','기학생 정보를 다시 등록할 수 없습니다!')
            return
        else:
            print('DB입력 진행!')
            values = (std_name, std_mobile, std_regyear) # 변수값 3개를 튜플변수 담고
            if self.addData(values): # 튜플을 파라미터로 전달
    ...
    def makeTable(self,lst_student):
        self.tblStudent.setColumnCount(QAbstractItemView.SingleSelection) # 단일 row 선택모드
        ...
        for i, (std_id, std_name, std_mobile, std_regyear) in enumerate(lst_student):
            self.tblStudent.setItem(i, 0, QTableWidgetItem(str(std_id))) # oracle number타입은 뿌릴 때 str()로 형변환 필요!
