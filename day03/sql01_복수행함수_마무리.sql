 ...
 * */
-- GROUP BY, HAVING
-- 그룹핑시 GROUP BY에 들어가는 컬럼만 SELECT 절에 사용 가능
...
-- ROW_NUMBER() - 행번호
-- 전체 employees에서 급여가 높은 사람부터 순위 매길때
SELECT first_name || ' ' || last_name AS "full_name"
	, salary
	, department_id
	, rank() OVER (order BY salary desc) AS "RANK"
	, DENSE_RANK() OVER (order BY salary desc) AS "DENSSE_RANK" -- 일상에서 DENSE_RANK 더 많이 사용
	, row_number() OVER (ORDER BY salary desc) AS "ROW_NUM" 
	FROM employees
	WHERE salary IS NOT NULL;
	
-- 부서별(department_id), 급여 높은 사람부터 랭킹을 매길때
SELECT first_name || ' ' || last_name AS "full_name"
	, salary
	, department_id
